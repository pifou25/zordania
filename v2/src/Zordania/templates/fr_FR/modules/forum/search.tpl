...
<if cond="isset({topic_pg})">
	...
	<if cond="empty({topic_pg})"><p class="infos">Aucun résultat.</p></if>
	<else>

            <p class="pages">
            <foreach cond="{topic_pg->links} as {page}">
                <if cond='is_numeric({page})'>
                    <a href="forum-search.html?search_id={search_id}&p={page}" title="page {page}">{page}</a>
                </if>
                <else>{page}</else>
            </foreach>
            </p>
		...
		<foreach cond="{topic_pg->get} as {topic}">
		<tr>
			<!-- Image pour l'état du topic -->
			<td>

			<if cond='{_user[ldate]} > {topic[posted_unformat]}'><set name="etat" value="lu" /></if>
...
<elseif cond="isset({post_pg})"><h4>Résultats de la recherche par posts</h4>
    <if cond="empty({post_pg})"><p class="infos">Aucun résultat.</p></if>
    <else>

        <p class="pages">
        <foreach cond="{post_pg->links} as {page}">
            <if cond='is_numeric({page})'>
                <a href="forum-search.html?search_id={search_id}&p={page}" title="page {page}">{page}</a>
            </if>
            <else>{page}</else>
...
<form id="search" method="get" action="forum-search.html">
    <fieldset>
        ...
        <div class="news">
            ...
            <p class="clearb">Pour effectuer une recherche par mots clés, saisissez le ou les termes à 
                rechercher. Séparez chaque terme avec des espaces. Utilisez AND, OR, NOT pour affiner 
                votre recherche. Pour rechercher par auteur, saisissez le nom d'utilisateur auquel les 
                messages appartiennent. Utilisez le joker * pour des recherches sur des mots incomplets.</p>
        </div>
    </fieldset>
