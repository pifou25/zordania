<if cond='!isset({unt_act}) || !{unt_act}'>
    ...
    <if cond='isset({unt_type})'>
        ...
        <table><tbody><tr><td>
            <if cond="empty({unt_dispo[bad][need_btc]})">
            ...
            </if><else><p class="error">Impossible de former cette unité car il manque le bâtiment nécessaire :
                <foreach cond='{unt_dispo[bad][need_btc]} as {btc_type1}'>
                    <a href="manual.html?race={_user[race]}&type=btc&value={btc_type1}" class="zrdPopUp" title="Batiment"><zimgbtc race="{_user[race]}" type="{btc_type1}" /></a>
                </foreach>
                </p></else>
        </td><td>
