...
<table class="liste">
  ...
  <br/>
  <if cond="empty({recap_array})">Il n'y a rien dans cette rubrique</if>
  <else>
	<foreach cond="{recap_array} as {fid}">
		<tr >
			<td><img src="img/forum/{fid[statut]}.png" title="{report_statut[{fid[statut]}]}" /></td>
			<td>{fid[poster]}</td>
			<td><a href="forum-<math oper="Template::str2url({fid[subject]})"/>.html?tid={fid[id]}" title="Nouvelle News!" >{fid[subject]}</a></td>
		</tr>
	</foreach>
</else>
</table>