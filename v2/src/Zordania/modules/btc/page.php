...
if($_act == 'btc')
{
	...
	//Gérer : formation / recherche / ressources dispo dans le bâtiment
		...
		if(!$btc_nb) { // aucun bât de ce type
			...
			include(MOD_DIR."btc/" .$_user['race']."/".$btc_type.".php");
			
			/* Autres trucs, en fonction de la conf */
            // former unité, liste des unités, annuler une formation
			if(isset($btc_conf['prod_unt'])) include(MOD_DIR."btc/inc/unt.php");
            // liste recherches, nouvelle recherche, annuler recherche en cours
			if(isset($btc_conf['prod_src'])) include(MOD_DIR."btc/inc/src.php");
            // idem ressources
			if(isset($btc_conf['prod_res'])) include(MOD_DIR."btc/inc/res.php");
            // tout le commerce (marché achat vente cours ...)
			if(isset($btc_conf['com'])) include(MOD_DIR."btc/inc/com.php");
            // la page "info" du bat
			include(MOD_DIR."btc/inc/info.php");
