...
case 'com': /* ajouter commentaire admin */
	...
	if($sid) {
		...
		$msg_infos = MsgRec::getSign(array('id'=>$sid));
		if(!$msg_infos)
...
case "read": /* Lecture */
	...
	if($sign_id) {
		$msg_infos = MsgRec::getSign(array('mrec_id'=>$sign_id));
...
case 'assign': /* s'assigner un msg signalé */
	...
	$_tpl->set("msg_assigne",Sign::assign($sid, $_user['mid'], $com));
...
default:
	if ($_act == 'maj') { /* maj du nb de messages signalés */
		$count_sign = Sign::count(array('admid' => 0));
		$admin_cache->msg_report = $count_sign['count_sign'];
		$admin_cache->force_save();
	}

	$_tpl->set("list_sign", MsgRec::getSign(array('etat'=>0)));
	$count_sign = Sign::count();
